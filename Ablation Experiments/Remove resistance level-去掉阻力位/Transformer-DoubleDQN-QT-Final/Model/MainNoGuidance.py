...
print(ModelType)
agent = Double_DQN(state_dim=state_dim, hidden_size=hidden_size,ModelType=ModelType,MEMORY_THRESHOLD=1000)
epoch = 7
    ...
    while env.TimeCursor < env.DataLen:
        ...
        if done == 0:
            ...
            if guidanceFlag==1:
                ...
                action = agent.action(obs, israndom=True,ResistancePointFlag=0,hold_time=env.hold_time)
            #print(action)
            ...
            agent.learn(obs, action, reward, new_obs, done,ResistancePointFlag=0,hold_time=env.hold_time)
            obs = new_obs
            episode_timesteps += 1
            total_timesteps += 1
