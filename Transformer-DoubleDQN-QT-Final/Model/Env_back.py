...
class Env():
    def __init__(self,data_path):
        ...
        self.Data['HV']=0.0#波动率
        #self.Data=self.DataAll.loc[:,['high','low','open','avg','vol','amount','time']]
        self.InitDay=25#不能低于ModelWindow+BeforeVOLN
        ...
        self.ResistancePoint=[[self.ResistanceCursor,self.Data.loc[self.ResistanceCursor,'time'],self.Data.loc[self.ResistanceCursor,'avg']]]#阻力位
        self.SupportPoint=[[self.ResistanceCursor,self.Data.loc[self.ResistanceCursor,'time'],self.Data.loc[self.ResistanceCursor,'avg']]]#支撑位
        self.transfer = TransferData(Data=self.Data, TimeCursor=self.TimeCursor, BeforeN=self.ModelWindow,BeforeVOLN=5)
        self.Observation = self.transfer.DataBuffer[['high','low','open','close','avg','vol','amount','PositionMarker','NextDay']]
        self.getResistanceSupport()

        #波动率
        self.loghv = LogHV(Data=self.Data, TimeCursor=self.TimeCursor, N=5)
    ...
    def getResistancePointFlag(self,beforeN):#价格运动到阻力位区间则发出1的信号
        bar=self.Data[self.TimeCursor,:]
    ...
    def step(self,action=0.6):#播放下一根k线，给模型看下一个时间状态,对应的奖励，以及该轮游戏是否结束
        ...
        if self.TimeCursor==self.DataLen:
            print("end")
            done=-2
            return self.Observation,reward,done

        TempObservation = self.transfer.OrdinaryToLog(self.TimeCursor)#由于是间接更新，当一个交易周期结束后并不影响里面的缓存，应该全部吧buffer里面的Positionmarket清0
        self.Observation = TempObservation[['high','low','open','close','avg','vol','amount','PositionMarker','NextDay']]
    ...
    def reward_fun(self,action):
        ...
        if action==0 and self.before_action==1:#平仓
            ...
            if bar['PositionMarker']<-0.12:#打止损平仓，动作正确，给予奖励
                reward=0.01
            else:
