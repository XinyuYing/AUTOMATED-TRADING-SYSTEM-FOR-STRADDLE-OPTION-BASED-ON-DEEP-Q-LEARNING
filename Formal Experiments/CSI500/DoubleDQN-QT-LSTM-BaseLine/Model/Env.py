...
#历史交易记录
        self.HistoricalTransaction = pd.DataFrame(columns=['OpenTime', 'OpenPoint', 'OpenAction',
                                                           'CloseTime', 'ClosePoint', 'ProfitRate'])
        self.Order = dict()
    ...
    def step(self, action=0.6):  # 播放下一根k线，给模型看下一个时间状态,对应的奖励，以及该轮游戏是否结束
        if abs(action-self.before_action)==2:#若要反手，先打平仓
            action=0
        bar = self.Data.loc[self.TimeCursor, :]
        ...
        if self.before_action == 1:  # 如果上一时刻为持有，更新浮盈浮亏
            self.Data.loc[self.TimeCursor, 'PositionMarker'] = math.log(self.account.getMarketValue(price=next_bar['close']) / self.account.OpenMarketValue)

        done = -1  # 游戏没开始
        reward = 0
        if action != 0 and self.before_action == 0:  # 开仓
            self.account.open(price=bar['close'], action=action)

            self.Order['OpenTime'] = bar['time']
            self.Order['OpenPoint'] = bar['close']
            self.Order['OpenAction']=action
            self.Data.loc[self.TimeCursor, 'PositionMarker'] = math.log(self.account.getMarketValue(price=next_bar['close']) / self.account.OpenMarketValue)
            self.hold_time = 1
            self.open_point = bar['close']
            reward = self.Data.loc[self.TimeCursor, 'PositionMarker']
            done = 0
        if action==0 and action != self.before_action:  # 平仓
            self.account.close(price=bar['close'])
        ...
        if action == self.before_action and action!=0:  # 持仓,
            reward = self.reward_fun(action)
            done = 0
    ...
    def reward_fun(self, action):
        ...
        if action == self.before_action and action==1:  # 持仓，浮盈浮亏作为奖惩
            bar = self.Data.loc[self.TimeCursor, :]
            beforeBar=self.Data.loc[self.TimeCursor-1, :]
            self.hold_time = self.hold_time + 1
            reward = bar['PositionMarker']-beforeBar['PositionMarker'] # 提示浮动盈亏
...
if __name__ == '__main__':
    ...
    env.step(action=1)
    env.step(action=1)
    env.step(action=0)  # 这一行执行有问题，Hv下降导致亏损
    env.step(action=1)
    env.step(action=1)
    env.step(action=-1)
    env.step(action=1)
