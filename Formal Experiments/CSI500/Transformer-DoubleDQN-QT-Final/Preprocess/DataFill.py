...
year='2022'
file_name='RESSET_INDXSH'+year+'_000905.csv'
hs300 = pd.read_csv('Data/A000905/' + file_name)
...
# 洗数据数据突然就没了,后边继续补,分为缺头部分和缺尾部分2019-4-29上午缺头，2018-8-29下午缺头，2020-2-3 下午缺尾，2019-5-28上午缺头
...
for date in date_range:
    ...
    for index, row in filtered_data.iterrows():
        ...
        next_data = hs300.loc[index + 1]
        next_time = next_data['time']
        if time < morning_end_time and next_time > afternoon_start_time:
            print("****" + str(next_time))
            minutes_difference = int((morning_end_time - time).total_seconds() / 60)
            avg_diff_price = (next_data['open'] - row['avg']) / minutes_difference
            new_price = row['avg']
            index1 = index
...
grouped_data = hs300.groupby(hs300['time'].dt.date).count()
hs300.to_csv('Data/B000905/'+file_name,index=False)